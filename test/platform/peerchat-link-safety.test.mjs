...
import {
  ...
  LINK_SUSPICIOUS,
  warnAboutLink
} from '../../app/peerchat/link-safety.mjs'
...
test('PeerChat leaves ordinary links alone', () => {
...
test('PeerChat flags a username stuffed in front of the real host', () => {
...
test('PeerChat flags a bare address instead of a site name', () => {
...
test('PeerChat flags a name built from lookalike characters', () => {
  assert.match(warnAboutLink('https://xn--pypal-4ve.com/login'), /imitate ordinary letters/)
})

test('PeerChat flags an address that reads as a filename', () => {
...
test('PeerChat flags a brand name outside the real domain', () => {
  for (const url of [
...
test('PeerChat matches a brand as a whole label, not a substring', () => {
...
test('PeerChat calls a shortener opaque rather than a scam', () => {
...
test('PeerChat ignores anything that is not an http link', () => {
...
test('PeerChat does not mistake a path for credentials', () => {
  // React Native's URL shim reads a password out of ':' and '@' anywhere in
  // the string, which would warn on both of these.
...
test('PeerChat pulls the first link out of a message without its punctuation', () => {
...
test('PeerChat says nothing when there is nothing to warn about', () => {
